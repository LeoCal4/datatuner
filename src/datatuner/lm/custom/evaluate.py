...
def main():
    ...
    test_loader = datatuner_dataset.get_data_loaders(
        ...
        dataset_types=["test"], batch_sizes={"test": args.test_batch_size})[0]
    ...
    with open(os.path.join(args.save_dir_path, "test_predictions.txt"), "w") as f:
        f.write(to_write)
    #* compute the average BLEU score and save them
    default_choice_avg_bleu = sum(default_choice_bleus)/float(len(default_choice_bleus))
    best_choice_avg_bleu = sum(best_choice_bleus)/float(len(best_choice_bleus))
    with open(os.path.join(args.save_dir_path, "test_stats.txt"), "w") as f:
        f.write(f"EVALUATION:\nAvg final BLEU: {default_choice_avg_bleu:.5f}\nAvg final best BLEU: {best_choice_avg_bleu:.5f}")
    log.info(f"Evaluation complete! Final avg BLEU: {default_choice_avg_bleu:.5f}")
